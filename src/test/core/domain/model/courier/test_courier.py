...
def test_courier_creation():
    ...
    courier = Courier.create(name="John", transport=transport, location=location)
...
def test_courier_steps_to_order():
    ...
    courier = Courier.create(name="John", transport=transport, location=courier_location)
...
def test_courier_private_constructor():
    with pytest.raises(RuntimeError, match="Please use named constructors!"):
        Courier(
            ...
            _id=uuid4(),
            _name="Name",
            _transport=Transport(name="string", speed=2),
